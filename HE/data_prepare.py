...
if not path_wd == '':
    os.chdir(path_wd)
need_save = False
def prepare_data(images_dir_root, size_square, image_suffix):
    num_name = 0
    img_data = []
    image_sum = 0
    img_label = []
    ...
    for image_dir in image_dir_list :
        ...
        for index_xml in range(len(xml_files)):
            ...
            image_address_list = glob(join(image_dir, image_suffix))  ## *.svs  *.ndpi
...
if __name__ == '__main__':
    ...
    if(args.prepare_types=="1"):
        ...
        prepare_data(args.images_dir_root, args.size_square, args.image_suffix)
