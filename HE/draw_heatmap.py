...
def draw_heatmap(image_address, save_model_address, size_square, begin_x, begin_y, dimensions_x, dimensions_y, step_x, step_y):
	...
	for slide_x in range(0,dimensions_x-size_square-1,step_x):
		print("slide_x:",slide_x," dimensions_x-size_square-1:",dimensions_x-size_square-1," dimensions_x:",dimensions_x)
		for slide_y in range(0,dimensions_y-size_square-1,step_y):
			...
			if ratio > 0.9: ###benben 0.2
				pred = [0]
                                ...
                                print("image_data dimention:",image_data.shape)

                                color_test_image = color(image_data, images_tmp)
                                test_image = np.reshape(color_test_image[0], (1, size_square, size_square, 3))
                                ...
                                print("begin getting the model saved before")
                                pred = CNN.model_test_single(save_model_address, test_image)[:,1]
                                print("--------------------pred:",pred)
                                ###############
...
if __name__ == '__main__':
        ...
        print("args.dimensions_x:",args.dimensions_x)
        if(args.pattern=="single"):
            draw_heatmap(args.image_address, args.save_model_address, args.size_square, args.begin_x, args.begin_y, args.dimensions_x, args.dimensions_y, args.step_x, args.step_y)
